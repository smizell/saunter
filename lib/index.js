...
function walk(
  initial,
  handlers = [],
  { initialResult, breakWhen = () => false } = {}
) {
  const result = new Result(initialResult);
  return walker(
    initial,
    initial,
    handlers,
    { initialResult, breakWhen },
    result,
    []
  );
}
  ...
  if (isWalkable(givenValue)) {
    ...
    lodash.each(givenValue, (value, keyOrIndex) => {
      ...
      lodash.each(handlers, handler => {
        ...
        if (
          handler.check({ value, result: currentResult, path: currentPath })
        ) {
          currentResult = handler.onTrue({
            value,
            path,
            initial,
...
module.exports = { walk, Result, Handler, Context, handlers };
